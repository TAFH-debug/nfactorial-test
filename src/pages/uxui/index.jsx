...
export default function Uxui() {
  ...
  const handleAnswer = (selectedOptionId) => {
    setAnswers((prevAnswers) => {
      const newAnswers = [...prevAnswers, selectedOptionId];

      if (currentQuestionIndex < questions.length - 1) {
        setCurrentQuestionIndex(currentQuestionIndex + 1);
      } else {
        // Теперь вызов submitAnswers происходит после обновления массива ответов
        submitAnswers(newAnswers); // Передаем новый массив ответов
      }

      return newAnswers; // Возвращаем обновленный массив для сохранения в состоянии
    });
  };
